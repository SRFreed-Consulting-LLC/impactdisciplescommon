...
import { FirebaseDAO, QueryParam, WhereFilterOperandKeys } from '../dao/firebase.dao';
...
export class EventService {
  ...
  queryAllByValues(queries: QueryParam[]): Promise<EventModel[]> {
    return this.dao.queryAllByMultiValue(this.table, queries).then(events => {
      events.forEach(event => {
        event.startDate = dateFromTimestamp(event.startDate as Timestamp);
        event.endDate = dateFromTimestamp(event.endDate as Timestamp);
